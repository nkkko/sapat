...
from .transcription.cloudflare import CloudflareTranscription
...
@click.option("--api", "-a", type=click.Choice(['openai', 'groq', 'azure', 'cloudflare'], case_sensitive=True), required=True, help="API to use for the transcription ('openai', 'groq', 'azure' or 'cloudflare')")
def main(input_path, language, prompt, temperature, quality, correct, api):
    ...
    elif api.lower() == "cloudflare":
        transcriber = CloudflareTranscription(temperature=temperature)
