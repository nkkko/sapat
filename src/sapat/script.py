...
from .transcription.local_whisper import LocalWhisperTranscription
...
@click.option("--api", "-a", type=click.Choice(['openai', 'groq', 'azure', 'local'], case_sensitive=True), required=True, help="API to use for the transcription ('openai', 'groq', 'azure' or 'local')")
def main(input_path, language, prompt, temperature, quality, correct, api):
    ...
    elif api.lower() == "local":
        transcriber = LocalWhisperTranscription(temperature=temperature)
    else:
