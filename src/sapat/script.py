...
@click.option("--api", "-a", type=click.Choice(['openai', 'groq', 'azure', 'whisperx'], case_sensitive=True), required=True, help="API to use for the transcription ('openai', 'groq', 'azure' or 'whisperx')")
def main(input_path, language, prompt, temperature, quality, correct, api):
    ...
    elif api.lower() == "whisperx":
        transcriber = WhisperXTranscription(temperature=temperature)
    else:
