...
from .transcription.revai import RevAITranscription
...
@click.option("--api", "-a", type=click.Choice(['openai', 'groq', 'azure', 'revai'], case_sensitive=True), required=True, help="API to use for the transcription ('openai', 'groq', 'azure' or 'revai')")
def main(input_path, language, prompt, temperature, quality, correct, api):
    ...
    elif api.lower() == "revai":
        transcriber = RevAITranscription(temperature=temperature)
    else:
