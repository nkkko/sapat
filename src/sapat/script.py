...
from .transcription.azure_speech import AzureSpeechTranscription
...
@click.option("--api", "-a", type=click.Choice(['openai', 'groq', 'azure', 'azurespeech'], case_sensitive=True), required=True, help="API to use for the transcription ('openai', 'groq', 'azure' or 'azurespeech')")
def main(input_path, language, prompt, temperature, quality, correct, api):
    ...
    elif api.lower() == "azurespeech":
        transcriber = AzureSpeechTranscription(temperature=temperature)
