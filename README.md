...
This tool automates the process of transcribing video files using multiple transcription services: Azure OpenAI, Groq, OpenAI APIs, and local WhisperX. It converts video files to MP3 format, transcribes the audio, and saves the transcription as a text file.

## Features

- Converts video files to MP3 format using ffmpeg
- Supports transcription using Azure OpenAI, Groq, OpenAI APIs, and local WhisperX
...
- API access for one or more supported services:
  ...
  - Groq Cloud API
  - OpenAI API
- Optional local WhisperX setup:
  - Python 3.10+
  - `whisperx` installed and available on PATH
  - A Hugging Face access token if you enable diarization
...
- `--api`: Specify the API to use for transcription.
   ...
   - `--api whisperx` for local WhisperX transcription with word alignment
...
Local WhisperX example:

```
sapat my_video.mp4 --quality M --language en --api whisperx
```

To run diarization with WhisperX, set `WHISPERX_DIARIZE=true` and provide
`WHISPERX_HF_TOKEN` after accepting the required Hugging Face model terms.
...
This tool is designed for use with multiple APIs (Azure OpenAI, Groq, and OpenAI) plus local WhisperX. Ensure you have valid API credentials configured in the `.env` file and the necessary permissions and credits for the API service you plan to use. For WhisperX, ensure the `whisperx` command and selected model can run in your local environment.
