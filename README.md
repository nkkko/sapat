...
3. Create a `.env` file in the project root and add your API credentials:
   ...
   # Optional: override for a custom deployment. If unset, Sapat chooses
   # audio-prod for whisper-v3 and audio-turbo for whisper-v3-turbo.
   # FIREWORKS_API_ENDPOINT=https://audio-prod.api.fireworks.ai/v1/audio/transcriptions
   # Optional Fireworks ASR controls
   FIREWORKS_VAD_MODEL=silero
   FIREWORKS_ALIGNMENT_MODEL=mms_fa
...
- `--api`: Specify the API to use for transcription.
   ...
   - `--api fireworks` for Fireworks AI pre-recorded transcription
...
```
sapat my_video.mp4 --quality M --language en --prompt "Product demo with API names" --api fireworks
```
