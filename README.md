...
3. Create a `.env` file in the project root and add your API credentials:
   ...
   DEEPGRAM_CORRECTION_OPENAI_API_KEY=optional_openai_api_key_for_correction
   ```
...
When you use `--correct` with `--api deepgram`, Sapat uses `OPENAI_API_KEY` or `DEEPGRAM_CORRECTION_OPENAI_API_KEY` with `DEEPGRAM_CORRECTION_MODEL` to polish the transcript.
...
This tool is designed for use with multiple APIs (Azure OpenAI, Groq, OpenAI, and Deepgram). Ensure you have valid API credentials configured in the `.env` file and the necessary permissions and credits for the API service you plan to use.
